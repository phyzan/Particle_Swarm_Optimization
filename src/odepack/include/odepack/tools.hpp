...
template <typename T, size_t N> struct State
{
    T t;
    Array1D<T, N> vector;
    T habs;
...
template <typename T, size_t N> struct EventState
{
    T t;
    Array1D<T, N> true_vector;
    Array1D<T, N> exposed_vector;
    bool choose_true =
        true; // if true, then exposed_vector may contain garbage values. Do not read its values. if false, then
              // true_vector contains the true state vector, and exposed_vector contains the exposed state vector.
...
template <typename T> using FuncLike = void (*)(T *, const T &, const void *); // f(t, void) -> array

template <typename T>
using ObjFun = T (*)(const T &, const T *, const T *, const void *); // f(t, q, args, void) -> scalar

template <typename T> using ObjFunLike = T (*)(const T &, const void *); // f(t, void) -> scalar

template <typename T, size_t N> using JacMat = Array2D<T, N, N>;
...
template <typename T> T norm_squared(const T *x, size_t size)
{
    // optimize
    T res = 0;
...
template <typename T> bool resize_step(T &factor, T &habs, const T &min_step, const T &max_step)
{
    bool res = false;
...
template <typename T> inline bool is_finite(const T &value)
{
...
template <> inline bool is_finite(const mpfr::mpreal &value)
{
    return mpfr_number_p(value.mpfr_ptr()) != 0;
}

template <typename T> T rms_norm(const T *x, size_t size)
{
    return sqrt(norm_squared(x, size) / size);
}

template <typename T> T rms_norm(const T *x, const T *scale, size_t size)
{
    T norm_sq = 0;
...
template <typename T> T norm(const T *x, size_t size)
{
...
template <typename T> std::vector<T> subvec(const std::vector<T> &x, size_t start, size_t size)
{
    if (start >= x.size())
    {
...
template <typename T> bool all_are_finite(const T *data, size_t n)
{
...
template <typename T> std::vector<T> _t_event_data(const T *t, const EventMap &event_map, const std::string &event)
{
...
template <typename T> std::vector<T> bisect(ObjFunLike<T> f, const T &a, const T &b, const T &atol, const void *obj)
{
    T err = 2 * atol + 1;
    T _a = a;
    T _b = b;
...
template <class S> void mat_mat_prod(S *r, const S *a, const S *b, size_t m, size_t s, size_t n, const S &factor = 1)
{
    /*
    a : (m x s)
    b : (s x n)
...
template <class S> void mat_T_mat_prod(S *r, const S *a, const S *b, size_t m, size_t s, size_t n, const S &factor = 1)
{
    /*
    a : (s x m)
    b : (s x n)
...
{
    std::cout << "\033[2K\rProgress: " << std::setprecision(2) << n * 100. / target << "%" << " : " << n << "/"
              << target << "  Time elapsed : " << clock.message()
              << "      Estimated duration: " << format_duration(target * clock.seconds() / n) << std::flush;
}

template <typename T> struct OdeData
{
...
template <typename T, size_t N = 0> class OdeResult
{

  public:
    OdeResult(const std::vector<T> &t, const Array2D<T, 0, N> &q, EventMap event_map, bool diverges, bool success,
              double runtime, std::string message)
        : _t(t), _q(q), _event_map(std::move(event_map)), _diverges(diverges), _success(success), _runtime(runtime),
    ...
    template <std::integral INT1, std::integral INT2> const T &q(INT1 i, INT2 j) const
    {
        return _q(i, j);
    }
...
template <typename T> inline T choose_step(const T &habs, const T &hmin, const T &hmax)
{
    return std::max(std::min(habs, hmax), hmin);
}

template <class T, int N> struct ICS
{
    T t;
    Array1D<T, N> q;
};

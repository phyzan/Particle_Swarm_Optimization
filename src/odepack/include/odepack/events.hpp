...
During an ode integration, we might want to save specific events that are encoutered. For instance, we might want to
specify at exactly what time the function that is integrated reached a spacific value. The Event classes handle such
events, and are responsible for determining them with a desired accuracy.
...
template <typename T, size_t N> struct _EventObjFun;

template <typename T, size_t N> T event_obj_func(const T &t, const void *obj)
{
...
template <typename T, size_t N> class Event
{

    // The obj parameter in "determine" is passed inside any functions that are passed in an Event object.
    // These include the "q" function in determine, or others passed in the constructors that accept a const void*
    // parameter.
  ...
  public:
    ...
    bool determine(EventState<T, N> &result, const State<T, N> &before, const State<T, N> &after, FuncLike<T> q,
                   const void *obj)
    {
    ...
    virtual bool locate(T &t, const State<T, N> &before, const State<T, N> &after, FuncLike<T> q,
...
template <typename T, size_t N> class PreciseEvent : public Event<T, N>
{

  public:
    PreciseEvent(std::string name, ObjFun<T> when, int dir = 0, Func<T> mask = nullptr, bool hide_mask = false,
                 T event_tol = 1e-12, const void *obj = nullptr)
        : Event<T, N>(name, mask, hide_mask, obj), _when(when), _dir(sgn(dir)), _event_tol(event_tol)
    {
    }
    ...
    bool locate(T &t, const State<T, N> &before, const State<T, N> &after, FuncLike<T> q,
...
template <typename T, size_t N> class PeriodicEvent : public Event<T, N>
{

  public:
    PeriodicEvent(std::string name, T period, T start, Func<T> mask = nullptr, bool hide_mask = false,
                  const void *obj = nullptr);

    PeriodicEvent(std::string name, T period, Func<T> mask = nullptr, bool hide_mask = false,
                  const void *obj = nullptr);
...
template <typename T, size_t N> class TmaxEvent : public Event<T, N>
{
...
template <typename EventType, typename ObjType> class ObjectOwningEvent : public EventType
{
  ...
  public:
    ...
    ObjectOwningEvent(ObjectOwningEvent &&other) noexcept
        : EventType(std::move(other)), _object(std::move(other._object))
    {
        this->_obj = &_object;
    }
...
Event<T, N>::Event(std::string name, Func<T> mask, bool hide_mask, const void *obj)
    : _name(std::move(name)), _mask(mask), _hide_mask(hide_mask && mask != nullptr), _obj(obj)
{
    if (_name.empty())
    {
...
template <typename T, size_t N> bool Event<T, N>::is_stop_event() const
{
    return false;
}

template <typename T, size_t N> bool Event<T, N>::is_leathal() const
{
    return false;
}

template <typename T, size_t N> void Event<T, N>::set_args(const std::vector<T> &args)
{
    _args = args;
}

template <typename T, size_t N> inline std::string Event<T, N>::name() const
{
    return _name;
}

template <typename T, size_t N> inline bool Event<T, N>::is_masked() const
{
    return _mask != nullptr;
}

template <typename T, size_t N> inline bool Event<T, N>::hides_mask() const
{
    return _hide_mask;
}

template <typename T, size_t N> inline const std::vector<T> &Event<T, N>::args() const
{
    return _args;
}
...
template <typename T, size_t N> inline T PreciseEvent<T, N>::obj_fun(const T &t, const T *q) const
{
    return _when(t, q, this->args().data(), this->_obj);
}

template <typename T, size_t N>
bool PreciseEvent<T, N>::locate(T &t, const State<T, N> &before, const State<T, N> &after, FuncLike<T> q,
...
template <typename T, size_t N> inline const int &PreciseEvent<T, N>::dir() const
{
    return _dir;
}

template <typename T, size_t N> PreciseEvent<T, N> *PreciseEvent<T, N>::clone() const
{
...
template <typename T, size_t N>
PeriodicEvent<T, N>::PeriodicEvent(std::string name, T period, T start, Func<T> mask, bool hide_mask, const void *obj)
    : Event<T, N>(name, mask, hide_mask, obj), _period(period), _start(start)
{
    if (period <= 0)
    {
        throw std::runtime_error(
            "Period in PeriodicEvent must be positive. If integrating backwards, events are still counted.");
    }
}

template <typename T, size_t N>
PeriodicEvent<T, N>::PeriodicEvent(std::string name, T period, Func<T> mask, bool hide_mask, const void *obj)
    : PeriodicEvent<T, N>(name, period, inf<T>(), mask, hide_mask, obj)
{
}

template <typename T, size_t N> const T &PeriodicEvent<T, N>::t_start() const
{
    return _start;
}

template <typename T, size_t N> const T &PeriodicEvent<T, N>::period() const
{
    return _period;
}

template <typename T, size_t N> PeriodicEvent<T, N> *PeriodicEvent<T, N>::clone() const
{
    return new PeriodicEvent<T, N>(*this);
}

template <typename T, size_t N>
bool PeriodicEvent<T, N>::locate(T &t, const State<T, N> &before, const State<T, N> &after, FuncLike<T> q,
                                 const void *obj) const
{
    if (!is_finite(_start))
    {
...
template <typename T, size_t N> void PeriodicEvent<T, N>::set_start(const T &t)
{
    if (!is_finite(_start))
    {
        _start = t;
...
template <typename T, size_t N> bool discard_event(const Event<T, N> &event, const Event<T, N> &mark)
{
...
template <typename T, size_t N> class EventCollection
{

  public:
    template <typename EventIterator> EventCollection(const EventIterator &events)
    {
    ...
    EventCollection(EventCollection &&other) noexcept
        : _events(other._events), _names(std::move(other._names)), _states(other._states), _N_tot(other._N_tot),
          _N_detect(other._N_detect), _Nt(other._Nt), _canon_idx(other._canon_idx), _event_idx(other._event_idx),
          _event_idx_start(other._event_idx_start), _iter(other._iter)
    {
    ...
    void detect_all_between(const State<T, N> &before, const State<T, N> &after, FuncLike<T> q, const void *obj)
    {
        ...
        _N_detect =
            0; // this is how many events have been triggered, and is the next available index in _event_idx_start
    ...
    void set_start(T t0, int dir)
    {
        for (size_t i = 0; i < this->size(); i++)
        {
            if (auto *p = dynamic_cast<PeriodicEvent<T, N> *>(_events[i]))
            {
                ...
                {
                    throw std::runtime_error(
                        "The starting time of a periodic event cannot be set at the initial time of the ode solver.");
                }
            }
        }
    }
  ...
  private:
    ...
    template <class Iterator> void _clone_events(const Iterator &events)
    {
        _names.clear();
        for (size_t i = 0; i < _N_tot; i++)
        {
    ...
    size_t *_event_idx_start =
        nullptr; // _event_idx[_event_idx_start[i]] is the first event index of the i-th unique time

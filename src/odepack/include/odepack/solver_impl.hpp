...
#define MAIN_DEFAULT_CONSTRUCTOR(T, N)                                                                                 \
    OdeData<T> ode, const T &t0, const Array1D<T, N> &q0, T rtol, T atol,                                              \
        T min_step = 0, T max_step = inf<T>(), T first_step = 0, int dir = 1, const std::vector<T> &args = {},         \
          const std::vector<const Event<T, N> *> &events = {}

#define MAIN_CONSTRUCTOR(T, N)                                                                                         \
    OdeData<T> ode, const T &t0, const Array1D<T, N> &q0, T rtol, T atol, T min_step, T max_step, T first_step,        \
        int dir, const std::vector<T> &args, const std::vector<const Event<T, N> *> &events

#define SOLVER_CONSTRUCTOR(T, N)                                                                                       \
    std::string name, OdeData<T> ode, const T &t0, const Array1D<T, N> &q0, T rtol, T atol, T min_step, T max_step,    \
        T first_step, int dir, const std::vector<T> &args, const std::vector<const Event<T, N> *> &events
...
template <typename T, size_t N, typename Derived> inline void interp_func(T *res, const T &t, const void *obj);

template <typename T, size_t N, typename Derived> class DerivedSolver : public OdeSolver<T, N>
{
  ...
  public:
    ...
    inline std::unique_ptr<Interpolator<T, N>> state_interpolator(
        int bdr1, int bdr2) const; // virtual. return a dynamically allocated object
...
DerivedSolver<T, N, Derived>::DerivedSolver(SOLVER_CONSTRUCTOR(T, N))
    : OdeSolver<T, N>(), _ode(ode), _rtol(rtol), _atol(atol), _min_step(std::max(min_step, MIN_STEP)),
      _max_step(max_step), _args(args), _n(q0.size()), _name(std::move(name)), _events(events),
      _direction(dir == 0 ? 1 : sgn(dir)), _current_linked_interpolator(t0, q0)
{
    if (first_step < 0)
    {
...
template <typename T, size_t N, typename Derived> inline const T &DerivedSolver<T, N, Derived>::t() const
{
    return (_event_idx == -1) ? current_state().t : _events.state(_event_idx).t;
}

template <typename T, size_t N, typename Derived> inline const Array1D<T, N> &DerivedSolver<T, N, Derived>::q() const
{
...
template <typename T, size_t N, typename Derived> inline const T &DerivedSolver<T, N, Derived>::stepsize() const
{
    return current_state().habs;
}

template <typename T, size_t N, typename Derived> inline int DerivedSolver<T, N, Derived>::direction() const
{
    return _direction;
}

template <typename T, size_t N, typename Derived> inline const T &DerivedSolver<T, N, Derived>::rtol() const
{
    return _rtol;
}

template <typename T, size_t N, typename Derived> inline const T &DerivedSolver<T, N, Derived>::atol() const
{
    return _atol;
}

template <typename T, size_t N, typename Derived> inline const T &DerivedSolver<T, N, Derived>::min_step() const
{
    return _min_step;
}

template <typename T, size_t N, typename Derived> inline const T &DerivedSolver<T, N, Derived>::max_step() const
{
    return _max_step;
}
...
template <typename T, size_t N, typename Derived> inline size_t DerivedSolver<T, N, Derived>::Nsys() const
{
    return _n;
}

template <typename T, size_t N, typename Derived> inline size_t DerivedSolver<T, N, Derived>::Nupdates() const
{
    return _N;
}

template <typename T, size_t N, typename Derived> inline bool DerivedSolver<T, N, Derived>::diverges() const
{
    return _diverges;
}

template <typename T, size_t N, typename Derived> inline bool DerivedSolver<T, N, Derived>::is_running() const
{
    return _is_running;
}

template <typename T, size_t N, typename Derived> inline bool DerivedSolver<T, N, Derived>::is_dead() const
{
    return _is_dead;
}

template <typename T, size_t N, typename Derived> inline std::string DerivedSolver<T, N, Derived>::message() const
{
    return _message;
}

template <typename T, size_t N, typename Derived> inline SolverState<T, N> DerivedSolver<T, N, Derived>::state() const
{
    return SolverState<T, N>({this->t(), this->q(), this->stepsize()}, this->current_events(), this->diverges(),
                             this->is_running(), this->is_dead(), this->Nupdates(), this->message());
}
...
template <typename T, size_t N, typename Derived> inline std::string DerivedSolver<T, N, Derived>::name() const
{
    return _name;
}

template <typename T, size_t N, typename Derived> T DerivedSolver<T, N, Derived>::auto_step(const ICS<T, N> *ics) const
{
...
template <typename T, size_t N, typename Derived> inline OdeSolver<T, N> *DerivedSolver<T, N, Derived>::clone() const
{
...
template <typename T, size_t N, typename Derived> inline bool DerivedSolver<T, N, Derived>::is_interpolating() const
{
    return _interp_data;
}

template <typename T, size_t N, typename Derived> inline bool DerivedSolver<T, N, Derived>::at_event() const
{
    return _event_idx != -1;
}

template <typename T, size_t N, typename Derived> inline const State<T, N> &DerivedSolver<T, N, Derived>::ics() const
{
    return _states[0];
}

template <typename T, size_t N, typename Derived> inline bool DerivedSolver<T, N, Derived>::advance()
{
    if (this->_is_dead)
    {
        this->_warn_dead();
    ...
    if (_events.begin())
    {
        ...
        if (_interp_data && _requires_new_start())
        {
            ...
            {
                std::unique_ptr<Interpolator<T, N>> r =
                    std::unique_ptr<Interpolator<T, N>>(new LocalInterpolator<T, N>(this->t(), this->true_vector()));
...
template <typename T, size_t N, typename Derived> inline bool DerivedSolver<T, N, Derived>::advance_to_event()
{
    if (_events.size() == 0)
    {
        return false;
...
template <typename T, size_t N, typename Derived> void DerivedSolver<T, N, Derived>::stop(std::string text)
{
    _is_running = false;
    _message = (text == "") ? "Stopped by user" : text;
}

template <typename T, size_t N, typename Derived> void DerivedSolver<T, N, Derived>::kill(std::string text)
{
    _is_running = false;
    _is_dead = true;
...
template <typename T, size_t N, typename Derived> bool DerivedSolver<T, N, Derived>::resume()
{
    if (_is_dead)
    {
        _warn_dead();
...
template <typename T, size_t N, typename Derived> void DerivedSolver<T, N, Derived>::set_tmax(T tmax)
{
    _events.set_tmax(tmax);
    this->resume();
}

template <typename T, size_t N, typename Derived> void DerivedSolver<T, N, Derived>::start_interpolation()
{
    if (!_interp_data)
    {
        _interp_data = true;
        ...
        {
            int bdr1 = 1;
            if (at_event() && _events.canon_event() && (_events.state(_event_idx).t == _events.canon_state()->t) &&
                _events.canon_event()->hides_mask())
            {
                cli = LinkedInterpolator<T, N>(this->t(), this->q());
                bdr1 = -1;
            }
...
template <typename T, size_t N, typename Derived> void DerivedSolver<T, N, Derived>::stop_interpolation()
{
    _current_linked_interpolator = LinkedInterpolator<T, N>(this->t(), this->q());
    _interp_data = false;
}

template <typename T, size_t N, typename Derived> void DerivedSolver<T, N, Derived>::reset()
{
    _diverges = false;
    _is_dead = false;
    _N = 0;
...
template <typename T, size_t N, typename Derived> inline void DerivedSolver<T, N, Derived>::set_obj(const void *obj)
{
    _ode.obj = obj;
}
...
inline void DerivedSolver<T, N, Derived>::interp(T *result, const T &t) const
{
    ...
    {
        throw std::runtime_error("Cannot perform local interpolation at t = " + to_string_f(t) +
                                 " between the states t_1 = " + to_string_f(this->old_state().t) +
                                 " and t_2 = " + to_string_f(this->current_state().t));
    }
...
template <typename T, size_t N, typename Derived> inline void DerivedSolver<T, N, Derived>::re_adjust()
{
    THIS->re_adjust();
}

template <typename T, size_t N, typename Derived> void DerivedSolver<T, N, Derived>::_register_states()
{
...
template <typename T, size_t N, typename Derived> void DerivedSolver<T, N, Derived>::_initialize_events(const T &t0)
{
...
template <typename T, size_t N, typename Derived> inline void DerivedSolver<T, N, Derived>::_warn_dead() const
{
    std::cout << "\n" << "Solver has permanently stop integrating. Termination cause:\n\t" << _message << std::endl;
}

template <typename T, size_t N, typename Derived> inline void DerivedSolver<T, N, Derived>::_warn_paused() const
{
    std::cout << "\n"
              << "Solver has paused integrating. Please resume the integrator by any means to continue advancing "
                 "*before* doing so."
              << std::endl;
}

template <typename T, size_t N, typename Derived> inline bool DerivedSolver<T, N, Derived>::_requires_new_start() const
{
    return _events.canon_event() && (_events.canon_state()->t == this->t());
}

template <typename T, size_t N, typename Derived> inline bool DerivedSolver<T, N, Derived>::_equiv_states() const
{

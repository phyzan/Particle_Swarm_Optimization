...
template <typename T>
void coef_mat_interp(T *result, const T &t, const T &t1, const T &t2, const T *y1, const T *y2, const T *coef_mat,
...
template <typename T, size_t N> class Interpolator
{
...
template <typename T, size_t N> class LocalInterpolator : public Interpolator<T, N>
{
...
template <typename T, size_t N> class StandardLocalInterpolator final : public LocalInterpolator<T, N>
{
  ...
  public:
    ...
    StandardLocalInterpolator(const Array2D<T, N, 0> &coef_mat, T t1, T t2, const Array1D<T, N> &y1,
                              const Array1D<T, N> &y2, int left_bdr, int right_bdr);
...
class LinkedInterpolator : public Interpolator<T, N>
{
  ...
  public:
    ...
    void expand_by_owning(std::unique_ptr<Interpolator<T, N>> &&interpolant)
        requires _is_void;
...
template <typename T, size_t N = 0> class OdeSolution : public OdeResult<T, N>
{

  public:
    OdeSolution(const std::vector<T> &t, const Array2D<T, 0, N> &q, const EventMap &event_map, bool diverges,
                bool success, double runtime, std::string message, const Interpolator<T, N> &interpolator)
        : OdeResult<T, N>(t, q, event_map, diverges, success, runtime, message), _interpolator(interpolator.clone())
    {
    }
    ...
    OdeSolution(OdeResult<T, N> &&other, const Interpolator<T, N> &interpolator)
        : OdeResult<T, N>(std::move(other)), _interpolator(interpolator.clone())
    {
    }
    ...
    {
        return new OdeSolution<T, N>(*this);
    }
...
template <typename T>
Interval<T>::Interval(const T &a, const T &b, int left_bdr, int right_bdr)
    : _a(a), _b(b), _h(b - a), _left(sgn(left_bdr)), _right(sgn(right_bdr)), _dir(sgn(b - a))
{
    if (a == b)
    {
...
template <typename T> inline bool Interval<T>::contains(const T &t) const
{
    return (t == _a) ? _left != -1 : ((t == _b) ? _right != -1 : (_a * _dir < t * _dir) && (t * _dir < _b * _dir));
}

template <typename T> inline const T &Interval<T>::start() const
{
    return _a;
}

template <typename T> inline const T &Interval<T>::end() const
{
    return _b;
}

template <typename T> inline const T &Interval<T>::h() const
{
    return _h;
}

template <typename T> inline int Interval<T>::dir() const
{
    return _dir;
}

template <typename T> inline int Interval<T>::start_bdr() const
{
    return _left;
}

template <typename T> inline int Interval<T>::end_bdr() const
{
    return _right;
}

template <typename T> inline bool Interval<T>::is_point() const
{
    return _is_point;
}

template <typename T> void Interval<T>::open_end()
{
    if (_is_point)
    {
...
template <typename T> inline void Interval<T>::close_end()
{
    _right = 1;
}

template <typename T> inline void Interval<T>::close_start()
{
    _left = 1;
}

template <typename T> inline void Interval<T>::adjust_start(const T &new_start)
{
...
template <typename T> inline void Interval<T>::adjust_end(const T &new_end)
{
...
template <typename T> inline bool Interval<T>::can_link_with(const Interval<T> &other) const
{
    // boundaries and directions must match.
    return (_b == other._a) && (_right * other._left != 1) && (_right != 0 || other._left != 0) &&
           (_dir + other._dir != 0);
}

template <typename T> void Interval<T>::link_with(Interval<T> &other)
{
...
template <typename T> std::string Interval<T>::signature(int prec) const
{
    if (is_point())
    {
        return "[" + to_string_f(_a, prec) + "]";
    }
    else
    {
        std::string left = (_left == 1) ? "[" : (_left == -1 ? "(" : "|");
        std::string right = (_right == 1) ? "]" : (_right == -1 ? ")" : "|");
        return left + to_string_f(_a, prec) + ", " + to_string_f(_b, prec) + right;
    }
}

template <typename T, size_t N> Interpolator<T, N>::Interpolator(size_t n) : _array_size(n)
{
}

template <typename T, size_t N> void Interpolator<T, N>::call(T *result, const T &t) const
{
    if (this->is_out_of_bounds(t))
    {
        throw std::runtime_error("Scalar " + to_string_f(t, 8) + " is out of bounds in LinkedInterpolator with range " +
                                 interval().signature(8));
    }
    this->_call_impl(result, t);
}

template <typename T, size_t N> Array1D<T, N> Interpolator<T, N>::make_call(const T &t) const
{
...
template <typename T, size_t N> size_t Interpolator<T, N>::array_size() const
{
    return _array_size;
}

template <typename T, size_t N> std::unique_ptr<Interpolator<T, N>> Interpolator<T, N>::safe_clone() const
{
    return std::unique_ptr<Interpolator<T, N>>(this->clone());
}

template <typename T, size_t N>
LocalInterpolator<T, N>::LocalInterpolator(const T &t, const Array1D<T, N> &q)
    : Interpolator<T, N>(q.size()), _interval(t), _tmin(t), _tmax(t), _q_old(q), _q(q)
{
}

template <typename T, size_t N>
LocalInterpolator<T, N>::LocalInterpolator(T t1, T t2, const Array1D<T, N> &y1, const Array1D<T, N> &y2, int left_bdr,
                                           int right_bdr)
    : Interpolator<T, N>(y1.size()), _interval(t1, t2, left_bdr, right_bdr), _tmin(t1), _tmax(t2), _q_old(y1), _q(y2)
{
}

template <typename T, size_t N> inline const Interval<T> &LocalInterpolator<T, N>::interval() const
{
    return _interval;
}

template <typename T, size_t N> inline const Array1D<T, N> &LocalInterpolator<T, N>::q_start() const
{
    return _q_old;
}

template <typename T, size_t N> inline const Array1D<T, N> &LocalInterpolator<T, N>::q_end() const
{
    return _q;
}

template <typename T, size_t N> size_t LocalInterpolator<T, N>::order() const
{
    return 1;
}

template <typename T, size_t N> inline int LocalInterpolator<T, N>::start_bdr() const
{
    return _interval.start_bdr();
}

template <typename T, size_t N> inline int LocalInterpolator<T, N>::end_bdr() const
{
    return _interval.end_bdr();
}

template <typename T, size_t N> inline int LocalInterpolator<T, N>::dir() const
{
    return _interval.dir();
}

template <typename T, size_t N> inline const T &LocalInterpolator<T, N>::t_start() const
{
    return _interval.start();
}

template <typename T, size_t N> inline const T &LocalInterpolator<T, N>::t_end() const
{
    return _interval.end();
}

template <typename T, size_t N> inline bool LocalInterpolator<T, N>::is_out_of_bounds(const T &t) const
{
...
inline bool LocalInterpolator<T, N>::can_link_with(const Interpolator<T, N> &other) const
{
    ...
    {
        return (t_end() == other.t_end()) && (end_bdr() * other.start_bdr() != 1) &&
               !(end_bdr() == 0 && other.start_bdr() == 0) && (dir() + other.dir() != 0);
    }
}

template <typename T, size_t N> LocalInterpolator<T, N> *LocalInterpolator<T, N>::clone() const
{
    return new LocalInterpolator<T, N>(*this);
}

template <typename T, size_t N> void LocalInterpolator<T, N>::adjust_start(const T &t_start)
{
    int d = _interval.dir();
...
template <typename T, size_t N> void LocalInterpolator<T, N>::adjust_end(const T &t_end)
{
    int d = _interval.dir();
...
template <typename T, size_t N> void LocalInterpolator<T, N>::link_with(Interpolator<T, N> &other)
{
...
template <typename T, size_t N> void LocalInterpolator<T, N>::link_after(Interpolator<T, N> &other)
{
    other.link_with(*this);
}

template <typename T, size_t N> inline void LocalInterpolator<T, N>::close_end()
{
    _interval.close_end();
}

template <typename T, size_t N> inline void LocalInterpolator<T, N>::close_start()
{
    _interval.close_start();
}

template <typename T, size_t N> inline void LocalInterpolator<T, N>::open_end()
{
    _interval.open_end();
}

template <typename T, size_t N> const T &LocalInterpolator<T, N>::_t_min() const
{
    return _tmin;
}

template <typename T, size_t N> const T &LocalInterpolator<T, N>::_t_max() const
{
    return _tmax;
}

template <typename T, size_t N> void LocalInterpolator<T, N>::_call_impl(T *result, const T &t) const
{
    return lin_interp(result, t, _interval.start(), _interval.end(), _q_old.data(), _q.data(), this->array_size());
}

template <typename T, size_t N>
StandardLocalInterpolator<T, N>::StandardLocalInterpolator(const T &t, const Array1D<T, N> &q)
    : LocalInterpolator<T, N>(t, q)
{
}

template <typename T, size_t N>
StandardLocalInterpolator<T, N>::StandardLocalInterpolator(const Array2D<T, N, 0> &coef_mat, T t1, T t2,
                                                           const Array1D<T, N> &y1, const Array1D<T, N> &y2,
                                                           int left_bdr, int right_bdr)
    : LocalInterpolator<T, N>(t1, t2, y1, y2, left_bdr, right_bdr), _coef_mat(coef_mat), _order(coef_mat.Ncols())
{
}

template <typename T, size_t N> size_t StandardLocalInterpolator<T, N>::order() const
{
    return _order;
}

template <typename T, size_t N> StandardLocalInterpolator<T, N> *StandardLocalInterpolator<T, N>::clone() const
{
    return new StandardLocalInterpolator<T, N>(*this);
}

template <typename T, size_t N> void StandardLocalInterpolator<T, N>::_call_impl(T *result, const T &t) const
{
    return coef_mat_interp(result, t, this->_t_min(), this->_t_max(), this->q_start().data(), this->q_end().data(),
                           _coef_mat.data(), this->order(), this->array_size());
}

template <typename T, size_t N, typename INTERPOLATOR>
LinkedInterpolator<T, N, INTERPOLATOR>::LinkedInterpolator(const INTERPOLATOR *other)
    : Interpolator<T, N>(other->q_start().size()), _dir(other->dir())
{
...
template <typename T, size_t N, typename INTERPOLATOR>
LinkedInterpolator<T, N, INTERPOLATOR>::LinkedInterpolator(const T &t, const Array1D<T, N> &q)
    : Interpolator<T, N>(q.size())
{
    if constexpr (_is_void)
    {
...
template <typename T, size_t N, typename INTERPOLATOR>
LinkedInterpolator<T, N, INTERPOLATOR>::LinkedInterpolator(const LinkedInterpolator &other)
    : Interpolator<T, N>(other.q_start().size())
{
    LinkedInterpolator<T, N, INTERPOLATOR>::operator=(other);
}

template <typename T, size_t N, typename INTERPOLATOR>
LinkedInterpolator<T, N, INTERPOLATOR> &LinkedInterpolator<T, N, INTERPOLATOR>::operator=(
    const LinkedInterpolator &other)
{
    if (&other == this)
    {
        return *this;
...
template <typename T, size_t N, typename INTERPOLATOR> size_t LinkedInterpolator<T, N, INTERPOLATOR>::size() const
{
    if constexpr (_is_void)
    {
...
template <typename T, size_t N, typename INTERPOLATOR> int LinkedInterpolator<T, N, INTERPOLATOR>::start_bdr() const
{
    return _get(0).start_bdr();
}

template <typename T, size_t N, typename INTERPOLATOR> int LinkedInterpolator<T, N, INTERPOLATOR>::end_bdr() const
{
    return _get_last().end_bdr();
}

template <typename T, size_t N, typename INTERPOLATOR> inline int LinkedInterpolator<T, N, INTERPOLATOR>::dir() const
{
    return _dir;
}
...
template <typename T, size_t N, typename INTERPOLATOR>
void LinkedInterpolator<T, N, INTERPOLATOR>::expand_by_owning(std::unique_ptr<Interpolator<T, N>> &&interpolant)
    requires _is_void
{
    _interval_cached = false;
...
template <typename T, size_t N, typename INTERPOLATOR> inline void LinkedInterpolator<T, N, INTERPOLATOR>::open_end()
{
    _interval_cached = false;
    _get_last().open_end();
}

template <typename T, size_t N, typename INTERPOLATOR> inline void LinkedInterpolator<T, N, INTERPOLATOR>::close_end()
{
    _interval_cached = false;
    _get_last().close_end();
}

template <typename T, size_t N, typename INTERPOLATOR> inline void LinkedInterpolator<T, N, INTERPOLATOR>::close_start()
{
    _interval_cached = false;
    _get(0).close_end();
}
...
{
    // ideally we should check that the vectors match in the boundary, but this might be too expensive, so the user
    // should check it if necessary.
...
template <typename T, size_t N, typename INTERPOLATOR>
inline void LinkedInterpolator<T, N, INTERPOLATOR>::_throw_invalid_interpolant(
    const Interpolator<T, N> &interpolant) const
{
    throw std::runtime_error("Invalid interpolant in LinkedInterpolator. Cannot join intervals " +
                             _get_last().interval().signature() + " + " + interpolant.interval().signature());
}
#endif
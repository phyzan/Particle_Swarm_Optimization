...
{
  public:
    Simple_Poincare(const poinc_params<Type> &pc, std::ostream *output = &(std::cout)) : Parent_Poincare<Type, Type_Arr, Type_Vec>(pc, output)
    {
...
#pragma omp parallel for schedule(dynamic)
  ...
  private:
    ...
    static void objf_calculation_inline(int i, Simple_Poincare *instance, Type_Vec &fpopulation, const Type_Arr &population)
    {
        Type_Vec q(4);
        ...
        {
            fpopulation(i) = sqrt((q[0] - x) * (q[0] - x) + (q[2] - px) * (q[2] - px));
        }
        else
    ...
    static void objf_calculation(void *instance)
    {
        ...
        if (py > 0 && x != ode::inf<Type>())
        {
            (*l_instance->fpopulation)(l_instance->i) = sqrt((q[0] - x) * (q[0] - x) + (q[2] - px) * (q[2] - px));
        }
        else
        { // If py <= 0, return infinity.
